...
### Метод 1: Обычная установка (без xray-vpn)
...
### Метод 2: Установка с VPN (xray-vpn)
...
- редактировать VPN после установки (пункт меню `2`)
...
- **1) Установить**: установка ноды + создание/запуск `xray-vpn`
...
#### Обязательные настройки профиля Remnawave для метода 2

Идея: **Remnawave Node** отправляет трафик в локальный `xray-vpn` по SOCKS (`127.0.0.1:1080`), а `xray-vpn` решает, что вести через VPN, а что — напрямую.

В профиле Remnawave должны быть:
- **`outbounds`** с тегами (важен регистр): `VPNROUTER`, `DIRECT`, `BLOCK`
- `VPNROUTER` = socks `127.0.0.1:1080`
- `DIRECT` = `freedom`
- `BLOCK` = `blackhole`
...
Важно:
- `DIRECT` должен быть `freedom`.
- В `routing` используйте ровно те же теги, что в `outbounds` (`BLOCK`, а не `block`).
